...
const Services = () => {
  ...
  const brands = ["Amazon", "Meesho", "Flipkart", "Glowroad", "JioMart", "Amazon Global"];
  const details = [
  ...
  return (
    <div className="bg-[#2C4870] pt-16 pb-12 relative">
      <div className="w-full max-w-[1340px] mx-auto px-4 sm:px-6 lg:px-10">
        <h1 className="text-center font-bold text-3xl sm:text-4xl md:text-5xl text-yellow-600">Our Services</h1>
          ...
          {brands.map((item, i) => (
            <div
              ...
              className={`border border-slate-500 rounded-full px-4 sm:px-6 md:px-8 py-2 cursor-pointer transition-all duration-300 text-sm sm:text-base
                ${selectedBrand === item ? "bg-yellow-600 text-black scale-105" : "text-white hover:bg-yellow-600 hover:text-black"}
              `}
            >
              {item}
            </div>
        ...
        <div className="relative mt-8">
          <button onClick={scrollLeft} className="hidden sm:flex absolute left-0 top-1/2 -translate-y-1/2 bg-white/15 hover:bg-white/30 text-white p-1.5 rounded-full transition-all duration-300 z-10">
            <ChevronLeft size={18} />
          </button>
          <button onClick={scrollRight} className="hidden sm:flex absolute right-0 top-1/2 -translate-y-1/2 bg-white/15 hover:bg-white/30 text-white p-1.5 rounded-full transition-all duration-300 z-10">
            ...
            <div className="flex gap-4 sm:gap-6 p-4 sm:p-8 min-w-max">
              {brands.map((brand, idx) => (
                <div
                  ...
                  `}
                >
                  <div className={`border-b-2 py-2 text-xl sm:text-2xl font-bold text-center mb-3
                    ${selectedBrand === brand ? "border-yellow-500" : "border-yellow-500 text-yellow-400"}
                  `}>
                    {brand}
                  </div>
                  <ul className="list-disc list-inside space-y-2 sm:space-y-3">
                    {details.map((item, i) => (
                      <li key={i} className={`${selectedBrand === brand ? "text-black" : "text-white"} text-xs sm:text-sm md:text-base`}>
                        {item}
                      </li>
                    ))}
                  </ul>
