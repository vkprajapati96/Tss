      ...
      <div className="w-full max-w-[1340px] mx-auto px-4 sm:px-6 lg:px-10">
        ...
        <div className="flex flex-wrap items-center justify-center gap-4 sm:gap-8 pt-8">
          {brands.map((item, i) => (
            <div
              key={i}
              className="border border-slate-500 text-white rounded-full px-4 sm:px-6 md:px-8 py-2 
              cursor-pointer hover:bg-yellow-600 hover:text-black transition-all duration-300 text-sm sm:text-base"
            >
              {item}
            </div>
          ))}
        ...
        <div className="relative mt-8">
          ...
          <button
            ...
            bg-white/15 hover:bg-white/30 text-white p-1.5 rounded-full transition-all duration-300 z-10"
          >
            <ChevronLeft size={18} />
          </button>
          ...
          <div ref={scrollRef} className="overflow-x-auto scroll-smooth scrollbar-hide">
            <div className="flex gap-4 sm:gap-6 p-4 sm:p-8 min-w-max">
              {brands.map((brand, idx) => (
                ...
                >
                  <div className="text-yellow-400 border-b-2 border-yellow-500 text-xl sm:text-2xl font-bold py-2 text-center mb-3">
                    {brand}
                  </div>
                  <ul className="list-disc list-inside space-y-2 sm:space-y-3">
                    {details.map((item, i) => (
                      <li key={i} className="text-white text-xs sm:text-sm md:text-base">
                        {item}
                      </li>
                    ))}
                  </ul>
